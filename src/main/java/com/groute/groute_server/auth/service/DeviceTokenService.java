 ...
 * 디바이스/계정 전환/{@code onTokenRefresh} 재호출 케이스를 모두 흡수한다. 동시 등록 race는 PostgreSQL native upsert({@code
 * INSERT ... ON CONFLICT})로 원천 차단된다.
 */
@Service
...
public class DeviceTokenService {
    ...
    /**
     * 디바이스 토큰 등록(atomic upsert).
     *
     * <p>호출 후엔 {@code (userId, pushToken, isActive=true)} 상태가 보장된다. 동시 호출 중 한 row로 수렴.
     */
    @Transactional
    public void register(Long userId, DevicePlatform platform, String pushToken) {
        ...
        deviceTokenRepository.upsertByPushToken(userId, platform.name(), pushToken);
    }
}
