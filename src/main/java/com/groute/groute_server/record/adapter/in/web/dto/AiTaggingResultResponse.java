...
public record AiTaggingResultResponse(
        ...
        @Schema(description = "잡 상태", example = "SUCCESS")
        JobStatus status,
     ...
     *
     * <p>star_tags는 primary_category가 동일한 여러 row로 구성되므로,
     * 첫 번째 row에서 primary_category를 가져오고 모든 row의 detail_tag를 수집한다.
     *
    ...
    public static AiTaggingResultResponse from(List<StarTag> tags) {
        ...
        List<String> detailTags = tags.stream()
                .map(StarTag::getDetailTag)
                .toList();
